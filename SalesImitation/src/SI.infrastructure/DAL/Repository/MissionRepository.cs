...
namespace SI.Infrastructure.DAL.Repository
{
    public class MissionRepository : IMissionRepository
    {
        ...
        public async Task<Mission> GetByLevel(int level)
        {
            string sql = "SELECT * From Missions Where Level = @Level;";
            Mission mission = null;
            using (var connection = Connection)
            {
                var res = await connection.QueryFirstOrDefaultAsync(sql, new { Level = level });
                if (res != null)
                {
                    mission = new Mission(res.ID,
                        res.Name,
